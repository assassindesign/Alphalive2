	...
	sawDownBuffer(1, 21984)
{
    ...
    (formatManager.createReaderFor(new MemoryInputStream (WaveTables::squarewave20500_wav,
                                                          WaveTables::squarewave20500_wavSize,
                                                          false)));
    ...
    (formatManager.createReaderFor(new MemoryInputStream (WaveTables::sawwave20500_wav,
                                                          WaveTables::sawwave20500_wavSize,
                                                          false)));
    
	sawReader->read(&sawBuffer, 0, sawReader->lengthInSamples, 0, true, false);
	
	ScopedPointer <AudioFormatReader> sawDownReader
    (formatManager.createReaderFor(new MemoryInputStream (WaveTables::sawdownwave20500_wav,
                                                          WaveTables::sawdownwave20500_wavSize,
                                                          false)));
...
double Oscillator::processSqr()
{
	...
	sharedMemory.enter();
	output = *squareBuffer.getReadPointer(0, currentSample);
    sharedMemory.exit();
...
double Oscillator::processSawUp()
{
	...
	sharedMemory.enter();
	output = *sawBuffer.getReadPointer(0, currentSample);
    sharedMemory.exit();
...
double Oscillator::processSawDown()
{
	...
	sharedMemory.enter();
	output = *sawDownBuffer.getReadPointer(0, currentSample);
    sharedMemory.exit();
